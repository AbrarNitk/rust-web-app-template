...
pub fn init_db_pool() {
    ...
    let connection_manager = r2d2_postgres::PostgresConnectionManager::new(db_url.parse().unwrap(), r2d2_postgres::postgres::NoTls);
    DB_POOL.set(
        Pool::builder().build(connection_manager).expect("Error in creating connection pool")
